...
if (isset($_POST['dangnhap'])) {
  ...
  $sql = "SELECT * FROM account WHERE NUMBERPHONE='" . $sodienthoai . "' AND PASSWORD='" . $password . "' LIMIT 1";
  $row = mysqli_query($mysqli, $sql);
  $count = mysqli_num_rows($row);

  if ($count > 0) {
    $row_data = mysqli_fetch_array($row);
    $_SESSION['IDAC'] = $row_data['IDAC'];
    $_SESSION['NUMBERPHONE'] = $row_data['NUMBERPHONE'];
    $_SESSION['IDPE'] = $row_data['IDPE'];
    $_SESSION['status'] = $row_data['status'];
    $_SESSION['PASSWORD'] = $row_data['PASSWORD'];
    $IDPE = $_SESSION['IDPE'];

    // Chuẩn bị truy vấn SQL
    $sql = "SELECT IDAB FROM detailpermission WHERE IDPE = '$IDPE'";
    ...
    if ($result->num_rows > 0) {
      ...
      $_SESSION['IDAB_array'] = $IDAB_array;
    } else {
    ...
    if ($_SESSION['status'] == 0) {
      ...
      $sql = "SELECT * FROM permission,detailpermission WHERE permission.IDPE = detailpermission.IDPE and IDAB = 32 LIMIT 1";
      $row = mysqli_query($mysqli, $sql);
      $count = mysqli_num_rows($row);
      if ($count > 0)
    ...
    echo 'document.addEventListener("DOMContentLoaded", function() {
            Swal.fire({
                icon: "error",
                title: "Lỗi",
                text: "Sai số điện thoại hoặc mật khẩu, xin vui lòng thử lại",
            });
        });';
    echo '</script>';
  }
